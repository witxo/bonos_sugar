...
class fact_FacturasViewRecibo extends InvoiceView{
  ...
  function display(){
	...
	if ($this->grupo == "Mobius")
	{
		$this->ss->assign("Logo",'themes/default/images/logo_mobius.png');
	}
	else
	{
    ...
    if($ruta_logo && file_exists($ruta_logo) ){
      $this->ss->assign("Logo",$sugar_config['fact_seller_info']['path_to_logo']);
    ...
    if($sugar_config['fact_seller_info']['invoice_footer']){
      $this->ss->assign("Pie",$sugar_config['fact_seller_info']['invoice_footer']);
        ...
        $this->ss->assign("alumno_c",$this->alumno_c);
    // IMPUESTOS
	...
	if ($this->grupo == "Mobius")
	{
		$this->ss->assign("NombreFacturador","Aulamobius");
		$this->ss->assign("Telefonos","91 085 46 31 - 640 282 316");
		$this->ss->assign("Web","www.aulamobius.es");
	}
	else
	{
		$this->ss->assign("Telefonos","91 511 37 23 - 644 057 727");
		$this->ss->assign("Web","www.ivema.es");
	}
	...
	$this->ss->assign("FechaHoy", $fecha_actual); 
  ...
  return "modules/fact_Facturas/templates/recibo.html.tpl"; 
