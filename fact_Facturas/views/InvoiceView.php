  ...
  function infoParties(){
    ...
    $this->buyerInfo = array(
      ...
      'CountryCode'=>'ESP',   // @todo Introducir Pais en "Cliente" según Facturae
         ...
         $sql2 = " select * from accounts_cstm where id_c = '".$account->id."';"; 
  ...
  function calcularInfo(){
    ...
    $this->infoInvoice=array(
      ...
      'TotalTaxesWithheld'=>$this->num(0,'TotalTaxesWithheld'), // Se calcula al repasar los items
	    ...
	    $sql2 = " select *".
           " from fact_facturas_cstm where id_c = '".$this->bean->id."';"; 
		   
		   //echo $sql2;
    $result2 = $this->bean->db->query($sql2);
	$row2 = $this->bean->db->fetchByAssoc($result2);

	$label = translate('modo_pago_list', '', $row2['modo_pago_c']);
	$this->pago=$label;
    
	$label = translate('grupo_list', '', $row2['grupo_c']);
	$this->grupo=$label;
