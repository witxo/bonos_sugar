...
class FacturasIngresoMobius extends DashletGenericChart
{
  ...
  /**
    ...
    {   
        ...
        $query =  'SELECT '.
				...
				'  clasificacion_c,'.
                  '  DATE_FORMAT(fact_facturas_cstm.fecha_contable_c,"%Y-%m") as m, '.
				  '  fact_facturas_cstm.ingreso_si_no_c as ingreso, '.
                  '  sum(amount) as total, '.
                  '  count(*) as fact_count '.
                  ' FROM fact_facturas INNER JOIN fact_facturas_cstm '.
				  ' ON  id = id_c '.
				  ' WHERE fact_facturas_cstm.fecha_contable_c >= DATE_FORMAT("'.$this->fcd_date_start.'", "%Y-%m-%d %H:%i:%s") '.
                  '  AND fact_facturas_cstm.fecha_contable_c <= DATE_FORMAT("'.$this->fcd_date_end.'", "%Y-%m-%d %H:%i:%s") '.
                  '  AND fact_facturas.deleted=0 AND ( fact_facturas_type=\'factura\'  OR fact_facturas_type=\'recibo\' OR fact_facturas_type=\'correccion\' )'.
                  '  AND ingreso_si_no_c != \'No\' '.
                  '  AND grupo_c = \'mobius\' '.
			  '  AND estado != \'elaborando\' '.
                  ' GROUP BY clasificacion_c ,DATE_FORMAT(fact_facturas_cstm.fecha_contable_c,"%Y-%m") ORDER BY m';	
